...
--
-- Why tokyonight instead of the vendored sitruuna.vim: tokyonight is an actively
...
-- (on_highlights) to sitruuna's exact colors. The old colors/sitruuna.vim stays
-- vendored as a fallback (`:colorscheme sitruuna`) for comparison.
--
-- The colorscheme is applied below in this spec's own config, only when
-- vim.g.active_theme == "tokyoSitruuna" (set in lua/reklai/init.lua).
local s = {
	fg = "#d1d1d1",
	fg_alt = "#a1a1a1",
	comment = "#5c6366",
...
return {
	...
	opts = {
		...
		on_highlights = function(hl)
			...
			hl.NormalFloat = { fg = s.fg, bg = s.bg }
			hl.FloatBorder = { fg = s.lighter_bg, bg = s.bg }
	...
	config = function(_, opts)
		...
		if vim.g.active_theme == "tokyoSitruuna" then
