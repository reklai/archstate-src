...
vim.opt.hlsearch = true -- Enable highlighting of search matches.
...
-- colors are set by the active theme spec (nordic.lua / tokyonightSitruuna.lua).
...
-- override needed).
vim.opt.laststatus = 0 -- Hide the bottom statusline.
vim.opt.winbar = " %{&filetype ==# 'oil' ? substitute(expand('%'),'^oil://','','') : expand('%:p')} %m%r%=%l:%c  %P " -- Top bar: absolute path (oil:// stripped in oil), modified/RO flags, line:col, file %.
